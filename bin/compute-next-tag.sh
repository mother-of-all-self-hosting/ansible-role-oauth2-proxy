...
#
# Tags look like `v<OAuth2-Proxy version>-<release>`:
#
# - if defaults/main.yml points at an OAuth2-Proxy version that has never been
#   released, the release counter restarts at 0 (`v7.15.4-0`)
# - otherwise the counter is incremented (`v7.15.4-1`), but only if something
...
# Anchored at the start of the line, so that the `# renovate:` annotation above
# the variable and the variables derived from it (such as
# `oauth2_proxy_container_image_tag`) cannot be picked up instead.
version="$(sed -nE 's|^oauth2_proxy_version:[[:space:]]*"?([^"[:space:]]+)"?.*$|\1|p' "$defaults_path" | head -n1)"

if [ -z "$version" ]; then
	echo >&2 "Could not determine the OAuth2-Proxy version from $defaults_path"
	exit 1
fi

# The version values already carry a leading `v` (e.g. `v7.15.4`),
